...
title GUTTYTECH - Build GuttyRL.exe (.NET 9)
color 0B

:: ============================================================================
::  build_exe.bat - Compila o GuttyRL.exe (console .NET 9, arquivo UNICO).
::  - Self-contained: o cliente NAO precisa ter .NET instalado.
::  - Single-file: voce manda so o GuttyRL.exe (templates embutidos dentro).
...
echo  [+] Compilando GuttyRL.exe ^(single-file^)... pode levar ~1 min na 1a vez.
...
if not exist "%ROOT%\dotnet\publish\GuttyRL.exe" (
    color 0E
...
copy /y "%ROOT%\dotnet\publish\GuttyRL.exe" "%ROOT%\GuttyRL.exe" >nul

color 0A
echo.
echo  [+] PRONTO: %ROOT%\GuttyRL.exe
